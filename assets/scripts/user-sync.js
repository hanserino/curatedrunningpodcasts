(function () {
    ...
    function mergeProgressMaps(local, remote) {
        ...
        Object.keys(keys).forEach(function (key) {
            var left = localMap[key];
            var right = remoteMap[key];
            if (!left) {
                merged[key] = right;
            } else if (!right) {
                merged[key] = left;
            } else {
                merged[key] = (left.u || 0) >= (right.u || 0) ? left : right;
            }
        });
    ...
    function mergeLastListened(local, remote) {
        if (!local) {
            return remote || null;
        }
        if (!remote) {
        ...
        return (local.u || 0) >= (remote.u || 0) ? local : remote;
    }
    ...
    function writeLocalLibrary(library) {
        try {
            localStorage.setItem(STORAGE_KEY_PROGRESS, JSON.stringify(library.listen_progress || {}));
            if (library.last_listened) {
    ...
    async function fetchRemoteLibrary(userId) {
        var response = await client
            .from('user_library')
            .select('listen_progress,last_listened,favorites,filter_prefs')
            .eq('user_id', userId)
            .maybeSingle();
    ...
    async function upsertRemoteLibrary(userId, library) {
        var response = await client.from('user_library').upsert(
            {
                user_id: userId,
                listen_progress: library.listen_progress || {},
                last_listened: library.last_listened || null,
    ...
    async function syncFromCloud() {
        ...
        try {
            ...
            var merged = {
                ...
                last_listened: mergeLastListened(local.last_listened, remote && remote.last_listened),
